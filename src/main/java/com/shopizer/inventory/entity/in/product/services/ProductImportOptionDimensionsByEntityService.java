...
public class ProductImportOptionDimensionsByEntityService {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(ProductImportOptionDimensionsByEntityService.class);
		
	public boolean handleDimensions(ProductRequestEntityData record, PersistableProduct product, ProductSpecification specs) {
		String sMethod = "handleDimensions";
		loggerDebugM(sMethod, "start");
		try {

			// sizes are required when loading a product
			String dimensions = record.getDimension();

			String W = convertDimension(record.getPackageWeight(), dimensions).toString();
			String L = convertDimension(record.getPackageLength(), dimensions).toString();
			String H = convertDimension(record.getPackageHeight(), dimensions).toString();
			loggerDebugM(sMethod, "W:" + W + " L" + L + " H:" + H);

			specs.setHeight(convertDimension(record.getPackageHeight(), dimensions));
			specs.setWidth(convertDimension(record.getPackageWidth(), dimensions));
			specs.setLength(convertDimension(record.getPackageLength(), dimensions));
			specs.setWeight(convertWeight(record.getPackageWeight()));
		} catch (Exception ex) {
